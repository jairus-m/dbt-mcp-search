"""
Export catalog.json artifact from dbt Cloud Admin API.
"""
...
async def main():
    """Fetch catalog.json from dbt Cloud Admin API and save locally."""
    ...
    logger.info(f"Fetching catalog.json for run {DBT_RUN_ID}")
    ...
    logger.info(
        f"Fetched catalog.json with {nodes_count} nodes, {sources_count} sources"
    )
    ...
    logger.info(f"Saved catalog.json to {output_file}")
