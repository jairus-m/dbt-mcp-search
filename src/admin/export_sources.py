"""
Export sources.json artifact from dbt Cloud Admin API.
"""
...
async def main():
    """Fetch sources.json from dbt Cloud Admin API and save locally."""
    ...
    logger.info(f"Fetching sources.json for run {DBT_RUN_ID}")
    ...
    logger.info(f"Fetched sources.json with {results_count} results")
    ...
    logger.info(f"Saved sources.json to {output_file}")
