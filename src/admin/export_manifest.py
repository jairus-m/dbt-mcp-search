"""
Export manifest.json artifact from dbt Cloud Admin API.
"""
...
async def main():
    """Fetch manifest.json from dbt Cloud Admin API and save locally."""
    ...
    logger.info(f"Fetching manifest.json for run {DBT_RUN_ID}")
    ...
    macros_count = len(data.get("macros", {}))
    logger.info(
        f"Fetched manifest.json with {nodes_count} nodes, "
        f"{sources_count} sources, {macros_count} macros"
    )
    ...
    logger.info(f"Saved manifest.json to {output_file}")
